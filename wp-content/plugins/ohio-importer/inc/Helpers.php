...
class Helpers {
	...
	const COMMON_TYPE_BITMASK = 0b1;
	const WPBAKERY_TYPE_BITMASK = 0b10;
	const ELEMENTOR_SECTIONS_TYPE_BITMASK = 0b100;
	const ELEMENTOR_CONTAINERS_TYPE_BITMASK = 0b1000;

	const COMMON_IMPORT_KEY = 'common';
	const WPBAKERY_IMPORT_KEY = 'wpbakery';
	const ELEMENTOR_SECTIONS_IMPORT_KEY = 'elementor:sections';
	const ELEMENTOR_CONTAINERS_IMPORT_KEY = 'elementor:containers';

	const COMMON_IMPORT_URL_KEY = 'import_file_url';
	const WPBAKERY_IMPORT_URL_KEY = 'import_wpbakery_file_url';
	const ELEMENTOR_SECTIONS_IMPORT_URL_KEY = 'import_elementor_file_url:sections';
	const ELEMENTOR_CONTAINERS_IMPORT_URL_KEY = 'import_elementor_file_url:containers';
	const WIDGETS_IMPORT_URL_KEY = 'import_widget_file_url';
	const OPTIONS_IMPORT_URL_KEY = 'import_options_file_url';
	const SLIDERS_IMPORT_URL_KEY = 'import_sliders_file_url';
	...
	private static function is_import_file_info_format_correct( $import_file_info ) {
		return ! empty( $import_file_info[Helpers::COMMON_IMPORT_URL_KEY] ) ||
			! empty( $import_file_info[Helpers::WPBAKERY_IMPORT_URL_KEY] ) ||
			! empty( $import_file_info[Helpers::ELEMENTOR_SECTIONS_IMPORT_URL_KEY] ) ||
			! empty( $import_file_info[Helpers::ELEMENTOR_CONTAINERS_IMPORT_URL_KEY] );
	}
	...
	public static function download_import_files( $import_file_info, $type = 'wpbakery' ) {
		...
		if ( !empty( $import_file_info[Helpers::COMMON_IMPORT_URL_KEY] ) ) {
			$import_file_info[Helpers::COMMON_IMPORT_URL_KEY] .= '/' . $purchase_code;
		}
		if ( !empty( $import_file_info[Helpers::WPBAKERY_IMPORT_URL_KEY] ) ) {
			$import_file_info[Helpers::WPBAKERY_IMPORT_URL_KEY] .= '/' . $purchase_code;
		}
		if ( !empty( $import_file_info[Helpers::ELEMENTOR_SECTIONS_IMPORT_URL_KEY] ) ) {
			$import_file_info[Helpers::ELEMENTOR_SECTIONS_IMPORT_URL_KEY] .= '/' . $purchase_code;
		}
		if ( !empty( $import_file_info[Helpers::ELEMENTOR_CONTAINERS_IMPORT_URL_KEY] ) ) {
			$import_file_info[Helpers::ELEMENTOR_CONTAINERS_IMPORT_URL_KEY] .= '/' . $purchase_code;
		}
		if ( !empty( $import_file_info[Helpers::WIDGETS_IMPORT_URL_KEY] ) ) {
			$import_file_info[Helpers::WIDGETS_IMPORT_URL_KEY] .= '/' . $purchase_code;
		}
		if ( !empty( $import_file_info[Helpers::OPTIONS_IMPORT_URL_KEY] ) ) {
			$import_file_info[Helpers::OPTIONS_IMPORT_URL_KEY] .= '/' . $purchase_code;
		}
		if ( !empty( $import_file_info[Helpers::SLIDERS_IMPORT_URL_KEY] ) ) {
			$import_file_info[Helpers::SLIDERS_IMPORT_URL_KEY] .= '/' . $purchase_code;
		}
		// / CUSTOM CODE
		...
		if (
			empty( $import_file_info[Helpers::COMMON_IMPORT_URL_KEY] ) &&
			empty( $import_file_info[Helpers::WPBAKERY_IMPORT_URL_KEY] ) &&
			empty( $import_file_info[Helpers::ELEMENTOR_SECTIONS_IMPORT_URL_KEY] ) &&
			empty( $import_file_info[Helpers::ELEMENTOR_CONTAINERS_IMPORT_URL_KEY] )
		) {
			...
			switch ( $type ) {
				case Helpers::WPBAKERY_IMPORT_KEY:
					$file_url_key = Helpers::WPBAKERY_IMPORT_URL_KEY;
					break;
				case Helpers::ELEMENTOR_SECTIONS_IMPORT_KEY:
					$file_url_key = Helpers::ELEMENTOR_SECTIONS_IMPORT_URL_KEY;
					break;
				case Helpers::ELEMENTOR_CONTAINERS_IMPORT_KEY:
					$file_url_key = Helpers::ELEMENTOR_CONTAINERS_IMPORT_URL_KEY;
					break;
				default:
					$file_url_key = Helpers::COMMON_IMPORT_URL_KEY;
					break;
			}

			$downloaded_files['content'] = $downloader->download_file( $import_file_info[$file_url_key], $content_filename );
		...
		if ( ! empty( $import_file_info[Helpers::WIDGETS_IMPORT_URL_KEY] ) ) {
			...
			$downloaded_files['widgets'] = $downloader->download_file( $import_file_info[Helpers::WIDGETS_IMPORT_URL_KEY], $widget_filename );
		...
		if ( ! empty( $import_file_info[Helpers::OPTIONS_IMPORT_URL_KEY] ) ) {
			...
			// Download the widgets import file.
			$downloaded_files['options'] = $downloader->download_file( $import_file_info[Helpers::OPTIONS_IMPORT_URL_KEY], $options_filename );
		...
		if ( ! empty( $import_file_info[Helpers::SLIDERS_IMPORT_URL_KEY] ) ) {
			...
			// Download the widgets import file.
			$downloaded_files['sliders'] = $downloader->download_file( $import_file_info[Helpers::SLIDERS_IMPORT_URL_KEY], $sliders_filename );
	 ...
	 */
	public static function get_demo_import_item_categories( $item, $categories ) {
		$sanitized_categories = array();

		if ( isset( $item['categories'] ) ) {
			foreach ( $item['categories'] as $category ) {
				$category_key = array_search( $category, $categories );
				$sanitized_categories[] = $category_key;
			}
		}
